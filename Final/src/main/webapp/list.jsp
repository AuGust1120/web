...
<head>
    ...
    <%
        ...
        //采用限制查询控制显示条数
        psta = conn.prepareStatement("select *from student limit ?,?");
        //设置第一个占位符（？）表示的内容
	 ...
	 <body background="img/bg3.jpg" style="background-repeat:no-repeat background-attachment:fixed;background-size:100% 100%; ">
		...
		<div class="body">
			...
			<div class="categries">
                ...
                <li>
                    <input  name="game" type="radio" id="game4"><label for="game4"><a href="mod.jsp">修改学生信息</a></label>
                </li>
            </ul>
			</div>
			<div class="runimage" bgcolor="white" >
                    <form>
						...
						[<a href="list.jsp?pageNow=1">首页</a>]
						<%--如果是下一页就当前页加1--%>
						[<a href="list.jsp?pageNow=<%=pageNow-1%>">上一页</a>]
						<%
						   //循环将我们的分页数打印到页面上
    						for (int i = 1; i <= pageCount; i++) {
						%>
						[<a href="list.jsp?pageNow=<%=i%>"><%=i%>
						</a>]
						<%
						    }
						%>
						<%--如果是上一页就当前页减1--%>
						[<a href="list.jsp?pageNow=<%=pageNow+1%>">下一页</a>]
						<%--如果是尾页就是pageCount--%>
						[<a href="list.jsp?pageNow=<%=pageCount%>">尾页</a>]</th>
                             </tr> 
                           </table>
					</form>
				</div>
